...
class ImageAnalysisService
  ...
  def build_user_context
    ...
    <<~CONTEXT
      ...
      -----------------------------------
      USER PREFERENCES & CONTEXT (RAG-Enhanced)
      -----------------------------------
      #{rag_context}
      
      IMPORTANT: Consider these user preferences when:
      - Identifying food items (respect dietary restrictions, allergies are CRITICAL)
      - Estimating portion sizes (apply portion modifier based on user history)
      - Calculating calories and macros (adjust for preferences)
      - Providing dietary advice (align with user's health goals)
      - Referencing foods from user's history when relevant
  ...
  def build_rag_context
    ...
    Rails.logger.warn("[ImageAnalysis] RAG context failed: #{e.message}, falling back to basic context")
  ...
  def fetch_semantic_results(query)
    ...
    Rails.logger.warn("[ImageAnalysis] Semantic search failed: #{e.message}")
    []
  end
